...
import {
  PATTERN_MATCH_SETS,
  buildPatternMatchDeck,
} from '../../data/patternMatchData'
import Toast from '../Toast'

// ── Single card ───────────────────────────────────────────────────

function PatternMatchCard({ card, onClick, disabled }) {
  ...
  return (
    <button
      ...
      className="relative focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500/50"
      style={{
        width: '100%',
        aspectRatio: '1 / 1',
        perspective: 600,
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: 16,
            background: matched
              ? 'linear-gradient(145deg, #ff9f0a, #ff8c00)'
              : 'linear-gradient(145deg, #ff6b6b, #ff5252)',
            boxShadow: matched
              ? '0 4px 14px rgba(255,159,10,0.35)'
        ...
        <div
          style={{
            ...
            borderRadius: 16,
            background: matched
              ? 'linear-gradient(145deg, rgba(255,159,10,0.15), rgba(255,140,0,0.08))'
              : 'var(--bg-surface)',
            border: matched
              ? '1.5px solid rgba(255,159,10,0.45)'
              : '0.5px solid rgba(0,0,0,0.08)',
            boxShadow: matched
              ? '0 4px 14px rgba(255,159,10,0.2)'
              : 'var(--shadow-sm)',
            display: 'flex',
            alignItems: 'center',
            justifyContent: 'center',
      ...
      {PATTERN_MATCH_SETS.map((set) => {
        ...
        return (
          <button
            ...
            style={{
              background: active ? '#ff6b6b' : 'var(--fill-tertiary)',
...
export default function PatternMatchBoard() {
  const [selectedSet, setSelectedSet] = useState(PATTERN_MATCH_SETS[0])
...
function Game({ cardSet, onPlayAgain, selectedSet, onSetChange }) {
  ...
  const [deck, setDeck] = useState(() => buildPatternMatchDeck(cardSet))
  ...
  const handlePlayAgain = () => {
    setDeck(buildPatternMatchDeck(cardSet))
    setFlippedIds([])
    setMoves(0)
    setMatchedCount(0)
    setLocked(false)
