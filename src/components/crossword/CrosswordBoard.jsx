...
function CrosswordGame({ puzzle, onNewGame }) {
  ...
  const isClueCorrect = useCallback(
    (clueKey, currentLetters) => {
      ...
      return cells.every(
        ([r, c]) => currentLetters[r][c].toUpperCase() === puzzle.grid[r][c]
      )
    },
    [puzzle]
  )
    ...
    (currentLetters) => {
      for (let r = 0; r < 5; r++) {
        for (let c = 0; c < 5; c++) {
          ...
          if (currentLetters[r][c].toUpperCase() !== puzzle.grid[r][c])
            return false
        }
      }
      return true
  ...
  const getCellState = (row, col) => {
    ...
    if (activeClueCells.some(([r, c]) => r === row && c === col))
      return 'active-word'
    // Check if cell is part of any solved clue
    const { across, down } = getCluesForCell(row, col, puzzle)
    if ((across && solvedClues.has(across)) || (down && solvedClues.has(down)))
      return 'correct'
    return 'idle'
  }
        ...
        {puzzle.grid.map((row, r) =>
          row.map((answer, c) => (
            <Cell
              ...
              inputRef={
                activeCell[0] === r && activeCell[1] === c ? inputRef : null
              }
              ...
              row.map((answer, c) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 4,
                    background:
                      answer === null ? 'var(--label-primary)' : '#d1fae5',
