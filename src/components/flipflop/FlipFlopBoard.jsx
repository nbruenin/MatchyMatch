...
function FlipFlopTile({ tile, onClick, disabled, isFlipping: _isFlipping }) {
  const { id: _id, value, flipped, matched } = tile
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            style={{ fontSize: 'clamp(1rem, 2.5vw, 1.3rem)', opacity: 0.6 }}
          >
            ❓
          </span>
        </div>
...
export default function FlipFlopBoard() {
  ...
  const accuracy =
    totalAttempts === 0 ? 100 : Math.round((TOTAL_PAIRS / totalAttempts) * 100)
  ...
  return (
    <div className="flex flex-col items-center gap-5 w-full max-w-lg mx-auto px-4 sm:px-6 pt-4 pb-12">
      ...
      >
        Flip tiles to find all {TOTAL_PAIRS} matching pairs. Beat your best
        time!
      </p>
