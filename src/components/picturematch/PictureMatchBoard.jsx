...
function PictureMatchCard({ card, onClick, disabled }) {
...
export default function PictureMatchBoard() {
  const [selectedSet, setSelectedSet] = useState(PICTURE_MATCH_SETS[0])
...
function Game({ cardSet, onPlayAgain, selectedSet, onSetChange }) {
  ...
  const [deck, setDeck] = useState(() => buildPictureMatchDeck(cardSet))
  ...
  const handlePlayAgain = () => {
    setDeck(buildPictureMatchDeck(cardSet))
    setFlippedIds([])
    setMoves(0)
    setMatchedCount(0)
    setLocked(false)
