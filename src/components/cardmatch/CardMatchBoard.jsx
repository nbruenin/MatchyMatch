...
import { CARD_MATCH_SETS, buildCardMatchDeck } from '../../data/cardMatchData'
import Toast from '../Toast'

// ── Single card ───────────────────────────────────────────────────

function CardMatchCard({ card, onClick, disabled }) {
  ...
  return (
    <button
      ...
      className="relative focus:outline-none focus-visible:ring-2 focus-visible:ring-purple-500/50"
      style={{
        width: '100%',
        aspectRatio: '1 / 1',
        perspective: 600,
      ...
      >
        ...
        <div
          style={{
            ...
            borderRadius: 16,
            background: matched
              ? 'linear-gradient(145deg, #bf5af2, #a855f7)'
              : 'linear-gradient(145deg, #5e5ce6, #5856d6)',
            boxShadow: matched
              ? '0 4px 14px rgba(191,90,242,0.35)'
              : '0 4px 14px rgba(94,92,230,0.25)',
            display: 'flex',
            alignItems: 'center',
        ...
        >
          <span
            style={{ fontSize: 'clamp(1.1rem, 3vw, 1.5rem)', opacity: 0.5 }}
          >
            ❓
          </span>
        </div>
        ...
        <div
          style={{
            ...
            borderRadius: 16,
            background: matched
              ? 'linear-gradient(145deg, rgba(191,90,242,0.15), rgba(168,85,247,0.08))'
              : 'var(--bg-surface)',
            border: matched
              ? '1.5px solid rgba(191,90,242,0.45)'
      ...
      {CARD_MATCH_SETS.map((set) => {
        ...
        return (
          <button
            ...
            style={{
              background: active ? '#bf5af2' : 'var(--fill-tertiary)',
              color: active ? '#fff' : 'var(--label-secondary)',
              boxShadow: active ? '0 2px 8px rgba(191,90,242,0.3)' : 'none',
              border: 'none',
              cursor: 'pointer',
              fontSize: '0.82rem',
...
export default function CardMatchBoard() {
  const [selectedSet, setSelectedSet] = useState(CARD_MATCH_SETS[0])
...
function Game({ cardSet, onPlayAgain, selectedSet, onSetChange }) {
  ...
  const [deck, setDeck] = useState(() => buildCardMatchDeck(cardSet))
  ...
  const handlePlayAgain = () => {
    setDeck(buildCardMatchDeck(cardSet))
    setFlippedIds([])
    setMoves(0)
    setMatchedCount(0)
    setLocked(false)
